...
class AIEngine:
    ...
    def generate_simulation_response(self, text: str, persona: str) -> Dict[str, Any]:
        ...
        prompt = (f"Topic/context is implicit from the user's argument. You are {persona}, an expert debate opponent. "
                  f"Challenge the following argument in 2-4 concise sentences. Ask one sharp question. "
                  f"Do not invent citations or facts. User argument: {text}")
        live_reply = llm_service.generate(styles[persona], prompt)
        rebuttal = live_reply or f"{prefixes[persona]} {primary_counter['rebuttal_text']} {primary_counter['challenge_question']}"
        return {
