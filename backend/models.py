...
class User(Base):
    ...
    # The Debate Coach / Educator this Learner is assigned to, so a coach or
    # educator's dashboard only ever shows their own students, not every
    # learner on the platform. NULL for accounts with no mentor assigned yet.
...
class CoachFeedback(Base):
    """A message a Coach/Educator/Administrator sends to a specific
    student, usually starting from an AI-drafted suggestion that the
    sender can edit before sending."""
