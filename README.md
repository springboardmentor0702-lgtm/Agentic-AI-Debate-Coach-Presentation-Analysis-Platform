...
## Voice interaction
The debate arena and presentation lab support browser-based voice interaction using the Web Speech API (Chrome/Edge recommended). Spoken arguments are transcribed live into the debate composer, and AI opponent responses can be read aloud with browser text-to-speech. Microphone permission must be allowed for the site.

## Recommended local setup on Windows
Use Python 3.11, Node.js 18+ and PostgreSQL 18. Create `backend/.env` from `.env.example`, install the backend requirements in a Python virtual environment, then run `uvicorn main:app --reload --port 8000`. In a second terminal run `npm install` and `npm run dev` from `frontend/`.
