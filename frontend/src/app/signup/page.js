...
import { postJson, extractErrorMessage } from '../../lib/apiClient';
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      const { ok, data } = await postJson("/auth/register", {
        ...
        mentor_email: role === "Learner" && mentorEmail.trim() ? mentorEmail.trim() : null
      });

      if (!ok || !data) {
        throw new Error(extractErrorMessage(data, "Registration failed. Please check your details and try again."));
      }
      if (!data.access_token) {
        throw new Error("Sign-up response was missing an access token. Please try again.");
      }

      if (goal || topics) {
        await postJson(
          `/auth/profile/me?learning_goals=${encodeURIComponent(goal)}&preferred_topics=${encodeURIComponent(topics)}`,
          undefined,
          { method: "PUT", headers: { "Authorization": `Bearer ${data.access_token}` } }
        );
      }
  ...
  const handleGoogleOAuth = async () => {
    ...
    try {
      const { ok, data } = await postJson(
        `/auth/oauth2/login?provider=Google&email=${encodeURIComponent(email || 'user@gmail.com')}&role=${encodeURIComponent(role)}`,
        undefined,
        { headers: {} }
      );

      if (!ok || !data || !data.access_token) {
        throw new Error(extractErrorMessage(data, "Google sign up failed."));
      }
      ...
      setMessage({ type: 'error', text: e.message || "OAuth2 sign up failed." });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ minHeight: '90vh', background: '#F9FAFB', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '3rem 1.5rem', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ width: '100%', maxWidth: '460px', background: '#FFFFFF', borderRadius: 0, padding: '2.5rem 2rem', boxShadow: '0 20px 40px -15px rgba(0, 0, 0, 0.07)', border: '1px solid #E5E7EB', position: 'relative' }}>
        ...
        <form onSubmit={handleSignUp}>
          ...
          {/* Optional Mentor / Coach Assignment - only meaningful for a Learner account */}
          {role === 'Learner' && (
            <div style={{ marginBottom: '1.25rem' }}>
              <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#374151', marginBottom: '0.4rem' }}>Debate Coach / Educator Email (optional)</label>
              <input
                type="email"
                placeholder="e.g. kabir.coach.demo@gmail.com"
                value={mentorEmail}
                onChange={(e) => setMentorEmail(e.target.value)}
                style={{ width: '100%', padding: '0.85rem 1rem', borderRadius: 0, border: '1px solid #E5E7EB', outline: 'none', boxSizing: 'border-box', fontSize: '0.9rem' }}
              />
              <p style={{ fontSize: '0.78rem', color: '#9CA3AF', marginTop: '0.35rem' }}>If your Coach or Educator already has an account, enter their email here so they can see your progress. Leave blank if you don't have one yet.</p>
            </div>
          )}
