...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const { ok, data } = await postJson("/auth/login", { email: email.trim(), password });

      if (!ok || !data) {
        throw new Error(extractErrorMessage(data, "Authentication failed. Please check your email and password."));
      }
      if (!data.access_token) {
        throw new Error("Login response was missing an access token. Please try again.");
      }

      // Note: the account's real role always comes from the backend/JWT
      // (data.role), not from whatever this dropdown happens to show. We
      // used to reject the login whenever the dropdown didn't match the
      // stored role, which meant a correct email/password combo could be
      // rejected just because the dropdown had reset to its default. The
      // dropdown here is only used for the sign-up flow, so it's ignored
      // during login.
  ...
  const handleGoogleOAuth = async () => {
    ...
    try {
      const { ok, data } = await postJson(
        ...
        undefined,
        { headers: {} } // no JSON body for this endpoint; override the default JSON header
      );
      ...
      setMessage({ type: 'error', text: e.message || "OAuth2 login failed." });
    } finally {
      setLoading(false);
    }
  };
